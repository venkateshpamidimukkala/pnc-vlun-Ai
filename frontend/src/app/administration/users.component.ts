...
export class UsersComponent {
  ...
  changeRole(user: AuthUser, role: Role): void { this.auth.updateUserRole(user.user_id, role).subscribe({ next: updated => this.users = this.users.map(item => item.user_id === updated.user_id ? updated : item), error: () => this.error = 'Unable to update role.' }); }
}