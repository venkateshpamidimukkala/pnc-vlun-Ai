...
import { AuthService, Permission, Role } from './auth.service';
...
export const authGuard: CanActivateFn = (route) => {
  ...
  if (permissions && !permissions.every(permission => auth.hasPermission(permission))) return inject(Router).createUrlTree(['/dashboard']);
  return true;
};
