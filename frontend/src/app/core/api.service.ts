...
import { Observable, defer, filter, shareReplay, switchMap, take, tap, timeout, timer, throwError } from 'rxjs';
...
export class ApiService {
  ...
  createEngineWorkflow(payload: { project: string; repository_path: string; finding_ids?: string[] }): Observable<EngineWorkflow> { return this.timedRequest('POST /engine/workflows', this.http.post<EngineWorkflow>(`${this.baseUrl}/engine/workflows`, payload)).pipe(timeout({first: 30000}), tap(() => this.invalidateWorkflows())); }
  ...
  applicationInventory(): Observable<ApplicationInventoryGroup[]> { return this.http.get<ApplicationInventoryGroup[]>(`${this.baseUrl}/mvp/application-inventory`); }
